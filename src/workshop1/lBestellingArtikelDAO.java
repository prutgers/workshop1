 ...
 * @author Peter
 */
public class lBestellingArtikelDAO {
    
    
    // Toe te voegen is Statement.RETURN_GENERATED_KEYS zoals in KlantDAO
    
    public static void createKoppelBestellingArtikel(BestellingArtikel BestellingArtikel){
        String query = "INSERT INTO bestellingartikel (bestelling_id, artikel_id)"
                + " values (?, ?)";
        try(Connection con = new DBConnector().getConnection();){
            PreparedStatement pstmt = con.prepareStatement(query);
            pstmt.setInt(1, BestellingArtikel.getBestelling_id());
            pstmt.setInt(2, BestellingArtikel.getArtikel_id());
            pstmt.executeUpdate();
        }
